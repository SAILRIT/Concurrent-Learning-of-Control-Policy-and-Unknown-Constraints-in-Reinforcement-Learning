...
def GP_opt(episodes, spec_obs_template,operation, x_reg,y_reg,x_anom,y_anom,cost_limit,rng):
    ...
    space = [{'name': 'wall_x_pos', 'type': 'continuous', 'domain': tuple(rng[0])},
            {'name': 'wall_x_neg', 'type': 'continuous', 'domain': tuple(rng[1])}]
    ...
    spec_obs = spec_obs_template.format(vals[0],vals[1])
